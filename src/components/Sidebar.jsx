...
import { NavLink } from "react-router-dom";
import {
  FaHome, FaTachometerAlt, FaBox, FaChartLine, FaCalendarAlt,
  FaHistory, FaEnvelope, FaBell, FaCog, FaSignOutAlt,
  FaAngleDoubleLeft, FaUser, FaAngleDoubleRight
} from "react-icons/fa";
...
const Sidebar = () => {
  ...
  const iconStyle = { marginRight: collapsed ? "0" : "0.6rem", fontSize: "1.1rem" };
  const badgeStyle = {
    position: "absolute",
    right: collapsed ? "0.5rem" : 0,
    background: "red",
  ...
  const menuItemStyle = {
    display: "flex", alignItems: "center", marginBottom: "0.1rem",
    borderRadius: "0.5rem", padding: "0.5rem",
    justifyContent: collapsed ? "center" : "flex-start", position: "relative"
  };

  const activeStyle = { background: "#e6f0ff", color: "#2979ff" };
      ...
      <div style={sidebarStyle}>
        <div>
          <button onClick={() => setCollapsed((prev) => !prev)} style={{
            ...
            marginBottom: "0.1rem", fontSize: "1.1rem", color: "#2979ff"
          }}>
            {collapsed ? <FaAngleDoubleRight /> : <FaAngleDoubleLeft />}
          </button>

          {!collapsed && <h2 style={{ fontSize: "1.25rem", marginBottom: "0.1rem" }}>Sahakar Basket</h2>}
          ...
          {[{ icon: <FaHome />, label: "Home", to: "/" },
            ...
            { icon: <FaCalendarAlt />, label: "Schedules", badge: 2 },
            { icon: <FaHistory />, label: "History" }
          ].map((item, index) => item.to ? (
            <NavLink key={index} to={item.to}
              style={({ isActive }) => isActive
                ? { ...menuItemStyle, ...activeStyle, textDecoration: "none" }
                : { ...menuItemStyle, textDecoration: "none", color: "inherit" }
              }>
              <div style={iconStyle}>{item.icon}</div>
              {!collapsed && <span>{item.label}</span>}
              {item.badge && <span style={badgeStyle}>{item.badge}</span>}
        ...
        <NavLink to="/profile" style={{
          ...
          alignItems: "center", marginBottom: "85px", padding: "0.5rem",
          borderRadius: "0.5rem"
        }}>
          <img src="https://i.pravatar.cc/30" alt="User" style={{
            borderRadius: "50%", marginRight: collapsed ? 0 : "0.6rem"
          }} />
          {!collapsed && (
            <div>
              <div style={{ fontSize: "0.75rem", fontWeight: 600 }}>Michael Smith</div>
              <div style={{ fontSize: "0.6875rem", color: "#888" }}>michaelsmith12@gmail.com</div>
            </div>
          )}
        </NavLink>
      </div>
