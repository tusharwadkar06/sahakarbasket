...
import { RefreshButton, SearchInput } from '../components/ActionButtons';
...
const MerchantList = () => {
  ...
  const allSelected = selected.length === merchants.length && merchants.length > 0;

  // Handle master checkbox
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelected(merchants.map((_, idx) => idx));
    } else {
      setSelected([]);
    }
  };
              ...
              <tr>
                <th>
                  <input
                    ...
                    indeterminate={selected.length > 0 && selected.length < merchants.length ? "true" : undefined}
                    ref={input => {
                      if (input) {
                        input.indeterminate = selected.length > 0 && selected.length < merchants.length;
                      }
                    }}
                  />
                </th>
                <th><span role="img" aria-label="Merchant"></span> Merchant Name</th>
                ...
                <th><span role="img" aria-label="Action"></span> Action</th>
              </tr>
            </thead>
          </table>
